...
class PageRenderer {
  ...
  renderNavigation() {
    ...
    navLinks.innerHTML = nav[lang]
      .map((item, index) => `<li><a href="${navItems[index]}">${item}</a></li>`)
      .join("");
  }
  ...
  renderHero() {
    ...
    document.querySelector(".logo").textContent = this.loader.getText(
      basic.name,
    );
    document.querySelector(".hero h1").textContent = this.loader.getText(
      basic.name,
    );
    document.querySelector(".hero h2").textContent = this.loader.getText(
      basic.title,
    );
    document.querySelector(".hero-text > p").textContent = this.loader.getText(
      basic.description,
    );
  ...
  renderAbout() {
    ...
    const interestsTitle =
      lang === "zh" ? "我的研究兴趣包括：" : "My research interests include:";
  ...
  renderEducation() {
    ...
    timeline.innerHTML = experience.education
      .map(
        (edu) => `
  ...
  renderExperience() {
    ...
    const sectionTitle =
      lang === "zh" ? "工作实习经历" : "Work and Internship Experience";
    ...
    grid.innerHTML = experience.experience
      .map(
        (exp) => `
  ...
  renderPublications() {
    ...
    filterDiv.innerHTML = publications.categories[lang]
      .map((cat, idx) => {
    ...
    grid.innerHTML = publications.papers
      .map((paper) => {
  ...
  attachPublicationFilters() {
    const filterButtons = document.querySelectorAll(
      ".publications-filter .filter-btn",
    );
